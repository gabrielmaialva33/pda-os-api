...
export class Migration20221202232654 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table "collaborators" ("id" uuid not null default uuid_generate_v4(), "created_at" timestamptz(0) not null default now(), "updated_at" timestamptz(0) not null default now(), "deleted_at" timestamptz(0) null, "code" varchar(8) null, "cpf" varchar(14) null, "rg" varchar(18) null, "birth_date" timestamptz(0) null, "role" varchar(255) null, "job" varchar(100) null, "sex" text check ("sex" in (\'male\', \'female\', \'not_informed\')) not null default \'not_informed\', "work_type" text check ("work_type" in (\'clt\', \'pj\', \'freelancer\', \'internship\', \'not_informed\')) not null default \'not_informed\', "status" text check ("status" in (\'active\', \'inactive\')) not null default \'active\', "civil_status" text check ("civil_status" in (\'single\', \'married\', \'divorced\', \'widowed\', \'not_informed\')) not null default \'not_informed\', "description" text null, "user_id" uuid not null, constraint "collaborators_pkey" primary key ("id"));',
    );
    this.addSql(
    ...
    this.addSql(
      'alter table "collaborators" add constraint "collaborators_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade;',
    );

    this.addSql(
      'alter table "phones" add constraint "phones_collaborator_id_foreign" foreign key ("collaborator_id") references "collaborators" ("id") on update cascade;',
    );

    this.addSql(
      'alter table "addresses" add constraint "addresses_collaborator_id_foreign" foreign key ("collaborator_id") references "collaborators" ("id") on update cascade;',
    );
  ...
  async down(): Promise<void> {
    ...
    this.addSql(
      'alter table "phones" drop constraint "phones_collaborator_id_foreign";',
    );

    this.addSql(
      'alter table "addresses" drop constraint "addresses_collaborator_id_foreign";',
    );
